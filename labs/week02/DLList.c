...
void DLListBefore (DLList L, char *it)
{
	...
	// empty list
	if (L->first == NULL) { // or nitems == 0
	    L->first = new;
	    L->last = new;
	    L->curr = new;
	} else {
	    // current item is the first node
	    if (current == L->first) { // current == L->first
	        current->prev = new;
	        new->next = current;
	        L->first = new; 
	        L->curr = new;
	    // current item in the other places
	    } else { 
...
void DLListDelete (DLList L)
{
	...
	if (L->nitems == 1) { //current = first = last
	    L->first = NULL;
	    L->last = NULL;
	    L->curr = NULL;
	// current was last
