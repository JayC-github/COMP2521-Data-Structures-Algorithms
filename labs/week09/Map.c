...
int connections (Map g, LocationID start, LocationID end, TransportID type[])
{
	...
	VList head = g->connections[start];
	for (VList curr = head; curr != NULL; curr = curr->next) {
	    if (curr->v == end) { // curr->v = locationID
	        type[i] = curr->type;
	        i++;
	        connection++;
	    
	    // curr->v is a sea which means 'start' city is connected to sea
	    } else if (idToType (curr->v) == SEA && idToType(start) != SEA && idToType(end) != SEA) {
	        // to check wether the sea is connected to the 'end' city;
	        VList newhead = g->connections[curr->v];
	        VList newcurr;
	        for (newcurr = newhead; newcurr != NULL; newcurr = newcurr->next) {
	            if (newcurr->v == end) {
	                type[i] = BOAT;
	                i++;
	                connection++;
	...
	return connection;  // to keep the compiler happy
}
